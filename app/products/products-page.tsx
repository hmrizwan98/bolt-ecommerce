...
import { Product, SortOption, Category } from '@/lib/types'
...
const categories: Category[] = ['skincare', 'makeup', 'haircare', 'fragrance', 'tools']
...
export default function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  const [category, setCategory] = useState<Category | undefined>(
    (searchParams?.category as Category) || undefined
  )
  const [searchTerm, setSearchTerm] = useState(searchParams?.search || '')
  const [sort, setSort] = useState<SortOption>(
    (searchParams?.sort as SortOption) || 'popular'
  )
  ...
  useEffect(() => {
    ...
    if (minPrice > 0 || maxPrice < 100) {
      ...
      result = result.filter(
        product => product.price >= actualMinPrice && product.price <= actualMaxPrice
      )
    }
  ...
  const displayMinPrice = Math.round(lowestPrice + (priceSpread * (priceRange[0] / 100)))
  const displayMaxPrice = Math.round(lowestPrice + (priceSpread * (priceRange[1] / 100)))
    ...
    <div className="py-8 md:py-12">
      <div className="container-custom">
        <div className="flex flex-col mb-8">
          ...
          {/* Active Filters */}
          {(category || searchTerm || priceRange[0] > 0 || priceRange[1] < 100) && (
